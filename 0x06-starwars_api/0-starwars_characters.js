 ...
 * The first positional argument passed is the Movie ID, which corresponds to a specific Star Wars movie.
 * Displays one character name per line in the same order as the "characters" list in the /films/ endpoint.
 * Uses the Star Wars API (https://swapi-api.hbtn.io) to fetch movie and character data.
...
const fetchData = async () => {
  try {
    ...
    const characterPromises = characters.map(character => request(character));
    const characterResponses = await Promise.all(characterPromises);
    const characterData = characterResponses.map(response => JSON.parse(response));
